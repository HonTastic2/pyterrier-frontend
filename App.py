...
def summarize_query(query):
    ...
    messages=[
        {
        "role": "user",
        "content": "Summarize the following query to the 20 most informative terms, returning only the terms and no other characters or words, including the numbering of the terms: " + query
        }
    ]
    )
...
def post_data():
    ...
    if method == "llm":
        summary = summarize_query(title + " " + body)
        results = search_top_n(summary, body, "title", BM25, n)
    elif method == "mct":
        summary = most_common_terms(title + " " + body)
        results = search_top_n(summary, body, "title", BM25, n)
    else:
        summary = ""
    ...
    return jsonify({"result": results, "summary":summary}), 201
